...
def test_add_interface():
    r1,r2 = create_router()
    assert r1.interfaces == {"G0/0": {'ip': 'unassigned', 'status': 'shutdown', 'connect': []}}
    assert r2.interfaces == {"G0/1": {'ip': 'unassigned', 'status': 'shutdown', 'connect': []}}
...
def test_show_interfaces():
    ...
    assert r1.show_interfaces() == "Show Interfaces of R1\nR1 has 3 interfaces\nG0/0 IP address: unassigned\nG0/1 IP address: unassigned\nG0/2 IP address: unassigned\n"
